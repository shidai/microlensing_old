// calculate map of percentage contribution to event rate from NS
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
double g1 (double *k, size_t dim, void *params)
{
    ...
    vszfunc=(1.0/(sqrt(2.0*3.1415926)*sigma_vsz))*exp(-0.5*(vsz*vsz)/(sigma_vsz*sigma_vsz));
        ...
        F=2.77*pow(10.0,-8.0)*cos(b)*pnum*exp(-rds)*p_disk*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)/4.3656;
...
double g2 (double *k, size_t dim, void *params)
{
    ...
    vszfunc=(1.0/(sqrt(2.0*3.1415926)*sigma_vsz))*exp(-0.5*(vsz*vsz)/(sigma_vsz*sigma_vsz));
        ...
        F=2.77*pow(10.0,-8.0)*cos(b)*pnum*exp(-rds)*pnum2*exp(-rds2)*(Deff)*(sqrt(M)*mfunc)*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0)/4.3656;
...
{   
    ...
    vszfunc=(1.0/(sqrt(2.0*3.1415926)*sigma_vsz))*exp(-0.5*(vsz*vsz)/(sigma_vsz*sigma_vsz));
        ...
        F=2.77*pow(10.0,-8.0)*cos(b)*pnum*exp(-rds)*p_pulsar*(Deff)*(sqrt(M))*(v*vdxfunc*vdyfunc*vdzfunc*vsxfunc*vsyfunc*vszfunc)*1.02*pow(10.0,-9.0);
...
int main (int argc, char *argv[])
{
    ...
    {
        l=-12.0*3.1415926/180.0+0.24*3.1415926/180.0*i;

        for (b=-12.0*3.1415926/180.0;b<=0.0;b+=0.12*3.1415926/180.0)
        {
